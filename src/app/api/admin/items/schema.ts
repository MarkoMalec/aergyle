...
export const itemSchema = z.object({
  ...
  healingAmount: z.number().int().nullable().optional(),
...
type BalanceDb = Pick<
  ...
  | "itemStat"
  | "itemStatProgression"
  | "itemStatRarityOverride"
>;

/**
 * Reads the item form into the Item row and its balance tables. Creating and
 * editing use the same form, so both routes read it here. A balance table left
 * undefined was not sent and stays as it is. Throws an Error with a message for
 * the admin when a CSV block is malformed.
 */
export function parseItemInput(v: z.infer<typeof itemSchema>) {
  ...
  const equipTo = normalizeItemEquipTo(v.equipTo ?? null);
  ...
  const item = {
    ...
    seedGrowSeconds: isSeed ? toOptionalPositiveInt(v.seedGrowSeconds) : null,
    seedYieldItemId: isSeed ? toOptionalPositiveInt(v.seedYieldItemId) : null,
    seedYieldMin: isSeed ? toOptionalPositiveInt(v.seedYieldMin) : null,
    seedYieldMax: isSeed ? toOptionalPositiveInt(v.seedYieldMax) : null,
    seedHarvestSeconds: isSeed
      ? toOptionalPositiveInt(v.seedHarvestSeconds)
      : null,
    seedXp: isSeed ? toOptionalPositiveInt(v.seedXp) : null,
    healingAmount: supportsTimedEffect
      ? toOptionalPositiveInt(v.healingAmount)
      : null,
    foodEffectSeconds: supportsTimedEffect
      ? toOptionalPositiveInt(v.foodEffectSeconds)
      : null,
    equipTo,
    // Only weapons can be two-handed.
    twoHanded: equipTo === "weapon" && (v.twoHanded ?? false),
  ...
  // Items that cannot have a timed effect always end up with none.
  const foodEffectStats = !supportsTimedEffect
    ? []
    : typeof v.foodEffectStatsCsv === "string"
...
type ItemBalance = ReturnType<typeof parseItemInput>["balance"];

/**
 * Writes every balance table the form sent. With `replace` the item already
 * exists, so its current rows for those tables are removed first.
 */
export async function writeItemBalance(
  tx: BalanceDb,
  itemId: number,
  balance: ItemBalance,
  options: { replace: boolean },
) {
  const where = { where: { itemId } };
  const rows = <T extends object>(list: T[]) =>
    list.map((row) => ({ itemId, ...row }));

  if (balance.foodEffectStats) {
    if (options.replace) await tx.foodEffectStat.deleteMany(where);
    if (balance.foodEffectStats.length > 0) {
      await tx.foodEffectStat.createMany({
        data: rows(balance.foodEffectStats),
      });
    }
  }
  if (balance.toolEfficiencies) {
    if (options.replace) await tx.toolEfficiency.deleteMany(where);
    if (balance.toolEfficiencies.length > 0) {
      await tx.toolEfficiency.createMany({
        data: rows(balance.toolEfficiencies),
      });
    }
  }
  if (balance.itemStats) {
    if (options.replace) await tx.itemStat.deleteMany(where);
    if (balance.itemStats.length > 0) {
      await tx.itemStat.createMany({ data: rows(balance.itemStats) });
    }
  }
  if (balance.statProgressions) {
    if (options.replace) await tx.itemStatProgression.deleteMany(where);
    if (balance.statProgressions.length > 0) {
      await tx.itemStatProgression.createMany({
        data: rows(balance.statProgressions),
      });
    }
  }
  if (balance.statRarityOverrides) {
    if (options.replace) await tx.itemStatRarityOverride.deleteMany(where);
    if (balance.statRarityOverrides.length > 0) {
      await tx.itemStatRarityOverride.createMany({
        data: rows(balance.statRarityOverrides),
      });
    }
  }
}
