...
function EditableNumberCell(props: {
  ...
  const save = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/admin/gardening/seeds/${props.itemId}`, {
        method: "PATCH",
...
function EditableYieldItemCell(props: {
  seedId: number;
  value: GardeningSeedRow["seedYieldItem"];
  valueId?: number | null;
  options: YieldOption[] | null;
  onUpdated: (
    next: GardeningSeedRow["seedYieldItem"],
    nextId: number | null,
  ) => void;
}) {
  ...
  const onSelect = async (nextId: number | null) => {
    setIsSaving(true);
    try {
      const res = await fetch(`/api/admin/gardening/seeds/${props.seedId}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
...
export function GardeningSeedsTableClient(props: {
  initialSeeds: GardeningSeedRow[];
}) {
  const [seeds, setSeeds] = React.useState<GardeningSeedRow[]>(
    props.initialSeeds,
  );
  ...
  React.useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        const json: unknown = await res.json().catch(() => null);
        if (!res.ok) throw new Error(apiError(json, "Failed to load items"));
  ...
  const updateSeedYieldItem = React.useCallback(
    (
      ...
      setSeeds((prev) =>
        prev.map((s) =>
          s.id === id
            ? { ...s, seedYieldItem: next, seedYieldItemId: nextId }
            : s,
        ),
      );
    },
      ...
      <table className="min-w-full text-left text-sm">
        <thead className="text-white/70">
          <tr className="border-b border-white/10">
            ...
            <th className="px-3 py-2">Gardening XP</th>
              ...
              <tr key={s.id} className="border-b border-white/5">
                ...
                <td className="px-3 py-2">
                  <EditableNumberCell
                    ...
                    onUpdated={(v) =>
                      updateSeedField(s.id, "seedGrowSeconds", v)
                    }
                  />
                </td>
                <td className="px-3 py-2">
                  <EditableNumberCell
                    ...
                    display={formatSeconds}
                    onUpdated={(v) =>
                      updateSeedField(s.id, "seedHarvestSeconds", v)
                    }
                  />
                </td>
                <td className="px-3 py-2">
                ...
                <td className="px-3 py-2">
                  <EditableNumberCell
                    ...
                    field="seedXp"
                    value={s.seedXp}
                    onUpdated={(v) => updateSeedField(s.id, "seedXp", v)}
                  />
                </td>
                <td className="px-3 py-2">
                  {missing ? (
