...
**`tsc --noEmit` fails in CI.** Expected for now — `next.config.js` sets
`ignoreBuildErrors`, and the typecheck job is `continue-on-error: true` so it
reports without blocking. Tighten once the tree is clean.
